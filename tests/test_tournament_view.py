...
def mock_manager():
    ...
    manager.get_podium.return_value = {'first': 'Alice', 'second': 'Bob', 'third': 'Charlie'}
    return manager
...
def test_update_bracket_tree_finished_tournament(view, mock_manager):
    ...
    # Check visibility of frames
    assert not view.wb_frame.winfo_ismapped(), "Winners bracket should be hidden when podium is shown."
    assert not view.lb_frame.winfo_ismapped(), "Losers bracket should be hidden when podium is shown."
    assert view.podium_frame.winfo_ismapped(), "Podium should be visible when tournament is finished."
