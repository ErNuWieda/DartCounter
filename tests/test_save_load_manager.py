...
def slm_setup(monkeypatch):
    ...
    mock_player1.state = {'has_opened': True, 'hits': {}} # Muss ein serialisierbares Dict sein
    ...
    mock_player2.state = {'has_opened': True, 'hits': {}} # Muss ein serialisierbares Dict sein
...
def test_load_game_data_version_mismatch(mock_read_json, slm_setup):
    ...
    mock_ui_utils.show_message.assert_called_once_with('error', "Inkompatibler Spielstand", ANY, parent=mock_parent)
...
def test_restore_game_state(slm_setup):
    ...
    loaded_data = {
        ...
        'players': [
            {'name': 'P1_loaded', 'id': 1, 'score': 50, 'throws': [('T', 20, None)], 'stats': {'s1': 1}, 'state': {'hits': {'20': 1}}},
