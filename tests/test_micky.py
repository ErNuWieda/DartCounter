...
def test_initialization(player):
    ...
    assert player.next_target == "20"
    assert player.hits.get("20") == 0
...
def test_closing_target_advances_to_next(micky_logic, player):
    ...
    player.next_target = "20"
    player.hits["20"] = 2  # Bereits 2 Treffer

    status, _ = micky_logic._handle_throw(player, "Single", 20, [])

    assert player.hits["20"] == 3
    assert player.next_target == "19"
    assert status == "ok"
