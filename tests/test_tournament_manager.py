...
def test_initialization_creates_double_elim_structure(tm_4_players):
    ...
    assert len(tm.bracket['winners']) == 1
    assert len(tm.bracket['winners'][0]) == 2 # 4 players -> 2 matches
    assert len(tm.bracket['losers']) == 0
...
def test_get_next_match_priority(tm_4_players):
    ...
    tm.bracket['winners'][0] = [
        {'player1': p[0], 'player2': p[1], 'winner': None},
        {'player1': p[2], 'player2': p[3], 'winner': None}
    ]

    # Setup: WB R1 M1 ist gespielt, WB R1 M2 ist ungespielt. Der Verlierer von M1 erstellt LB R1 M1.
    match1 = tm.bracket['winners'][0][0]
    match2 = tm.bracket['winners'][0][1]

    # Vor jedem Match sollte das nächste Match das erste im WB sein
    assert tm.get_next_match() == match1

    # Spiele das erste Match
    tm.record_match_winner(match1, p[0])

    # Das nächste Match sollte das zweite Match im WB R1 sein
    assert tm.get_next_match() == match2

    # Spiele das zweite Match
    tm.record_match_winner(match2, match2['player1'])

    # Jetzt sollte das nächste Match im Losers Bracket sein
    assert len(tm.bracket['losers']) > 0
    assert len(tm.bracket['losers'][0]) == 1
    # Der Verlierer von Match 1 (p1) und Match 2 (p3) sind jetzt im Losers Bracket
    lb_players = {tm.bracket['losers'][0][0]['player1'], tm.bracket['losers'][0][0]['player2']}
    assert lb_players == {p[1], p[3]}
...
def test_record_match_winner_with_invalid_winner_raises_error():
    ...
    tm = TournamentManager(player_names=["Alice", "Bob"], game_mode="501")
...
def test_winner_advances_and_loser_drops(tm_4_players):
    ...
    assert len(tm.bracket['losers']) == 1
    lb_r1 = tm.bracket['losers'][0]
    assert len(lb_r1) == 1
    assert lb_r1[0]['player1'] == loser
    assert lb_r1[0]['player2'] is None  # Wartet auf anderen Verlierer
...
def test_get_tournament_winner_returns_none_if_not_finished(tm_4_players):
    ...
    assert tm_4_players.get_tournament_winner() is None
    assert tm_4_players.is_finished is False
...
def test_full_tournament_flow_4_players(tm_4_players):
    ...
    assert {tm.bracket['losers'][0][0]['player1'], tm.bracket['losers'][0][0]['player2']} == {p[1], p[3]} # B vs D
    ...
    assert len(tm.bracket['losers']) == 2
    lb_final = tm.get_next_match()
    # Die Teilnehmer sind der Gewinner aus der vorherigen LB-Runde (Bob/p[1])
    ...
    assert tm.get_tournament_winner() == p[0]
...
def test_bracket_reset_flow_4_players(tm_4_players):
    ...
    assert tm.is_finished is True
    assert tm.get_tournament_winner() == p[2]
...
def test_full_tournament_flow_8_players(tm_8_players):
    ...
    assert len(tm.bracket['winners']) == 2
    assert len(tm.bracket['losers']) == 1
    assert {tm.bracket['winners'][1][0]['player1'], tm.bracket['winners'][1][0]['player2']} == {p[0], p[2]}
    assert {tm.bracket['winners'][1][1]['player1'], tm.bracket['winners'][1][1]['player2']} == {p[4], p[6]}
    assert {tm.bracket['losers'][0][0]['player1'], tm.bracket['losers'][0][0]['player2']} == {p[1], p[3]}
    assert {tm.bracket['losers'][0][1]['player1'], tm.bracket['losers'][0][1]['player2']} == {p[5], p[7]}
    ...
    assert len(tm.bracket['winners']) == 3 # WB R1, R2, Finale
    assert len(tm.bracket['losers']) == 2  # LB R1, R2
    assert tm.bracket['winners'][2][0]['player1'] == p[0] and tm.bracket['winners'][2][0]['player2'] == p[4]
    # LB R2: Gewinner LB R1 (p1, p5) vs Verlierer WB R2 (p2, p6)
    assert {tm.bracket['losers'][1][0]['player1'], tm.bracket['losers'][1][0]['player2']} == {p[1], p[2]}
    assert {tm.bracket['losers'][1][1]['player1'], tm.bracket['losers'][1][1]['player2']} == {p[5], p[6]}
    ...
    assert len(tm.bracket['losers']) == 3
    assert {tm.bracket['losers'][2][0]['player1'], tm.bracket['losers'][2][0]['player2']} == {p[2], p[6]}
    ...
    assert {tm.bracket['losers'][3][0]['player1'], tm.bracket['losers'][3][0]['player2']} == {p[2], p[4]}
