...
from tests.mocks import MockPlayer, MockGame
...
class TestCricketLogic(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.player1_cricket = MockPlayer("Alice", game_name="Cricket")
        self.player1_cricket.state['hits'] = {target: 0 for target in CRICKET_TARGET_VALUES.keys()}
        self.player2_cricket = MockPlayer("Bob", game_name="Cricket")
        self.player2_cricket.state['hits'] = {target: 0 for target in CRICKET_TARGET_VALUES.keys()}
        ...
        self.player1_ct = MockPlayer("Charlie", game_name="Cut Throat")
        self.player1_ct.state['hits'] = {target: 0 for target in CRICKET_TARGET_VALUES.keys()}
        self.player2_ct = MockPlayer("David", game_name="Cut Throat")
        self.player2_ct.state['hits'] = {target: 0 for target in CRICKET_TARGET_VALUES.keys()}
    ...
    def test_closing_target(self):
        ...
        self.assertEqual(player.state['hits']["20"], 0)
        ...
        self.assertEqual(player.state['hits']["20"], 3)
    ...
    def test_cricket_no_scoring_when_opponent_closed(self):
        ...
        self.player1_cricket.state['hits']["20"] = 3
        # Bob schließt die 20 ebenfalls
        self.player2_cricket.state['hits']["20"] = 3
    ...
    def test_win_condition_cricket(self):
        ...
        for target in self.cricket_logic.targets:
    ...
    def test_win_condition_cut_throat(self):
        ...
        for target in self.cut_throat_logic.targets:
            player.state['hits'][target] = 3
