...
@pytest.mark.parametrize("score, darts_left, opt_out, expected", [
...
def test_get_checkout_suggestion_with_preferred_double(score, darts_left, preferred_double, expected):
    ...
    """
    suggestion = CheckoutCalculator.get_checkout_suggestion(score, "Double", darts_left, preferred_double=preferred_double)
    assert suggestion == expected
...
def test_load_checkout_paths_handles_bad_data(monkeypatch):
    ...
    monkeypatch.setattr(checkout_calculator, 'JsonIOHandler', MagicMock(read_json=MagicMock(return_value=mock_data)))
...
def test_load_checkout_paths_handles_no_file(monkeypatch):
    ...
    """
    monkeypatch.setattr(checkout_calculator, 'JsonIOHandler', MagicMock(read_json=MagicMock(return_value=None)))
    paths = checkout_calculator._load_checkout_paths()
    assert paths == {}