...
def test_draw_bracket_with_winner(bracket_canvas, simple_bracket_data):
    ...
    bracket_canvas.draw_bracket(simple_bracket_data, next_match=None, bracket_winner=final_winner)
...
def test_draw_bracket_with_next_match_highlight(bracket_canvas, simple_bracket_data):
    ...
    bracket_canvas.draw_bracket(simple_bracket_data, next_match=next_match_to_play, bracket_winner=None)
    ...
    for item_id in bracket_canvas.find_all():
        if bracket_canvas.type(item_id) == 'rectangle':
            if bracket_canvas.itemcget(item_id, 'fill') == bracket_canvas.NEXT_MATCH_COLOR:
                highlight_rects_found.append(item_id)

    assert len(highlight_rects_found) == 1, "Exactly one highlight rectangle should be drawn for the next match."

def test_draw_bracket_with_bye(bracket_canvas):
    """Tests if a BYE is drawn with the correct styling."""
    bracket_data = [
        [
            {'player1': 'Alice', 'player2': 'BYE', 'winner': 'Alice'}
        ]
    ]
