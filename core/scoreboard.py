...
class TargetBasedScoreBoard(BaseScoreBoard):
    ...
    def _create_extra_widgets(self, main_frame):
        ...
        for i, target in enumerate(self.player.targets):
            ...
            if self.game.options.name in ("Cricket", "Cut Throat", "Tactics", "Micky Mouse"):
...
def setup_scoreboards(game_controller):
    ...
    for i, player in enumerate(game_controller.players):
        if i < len(positions):
            ...
            sb = scoreboard_class(root=game_controller.dartboard.root, player=player, pos_x=pos_x, pos_y=pos_y, width=scoreboard_width, height=scoreboard_height)
