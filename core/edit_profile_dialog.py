...
class EditProfileDialog(tk.Toplevel):
    ...
    def _create_player_type_widgets(self, parent_frame):
        ...
        ttk.Radiobutton(
            type_frame,
            text="Mensch",
            variable=self.is_ai_var,
            value=False,
            command=self._on_player_type_change,
    ...
    def _create_double_out_widgets(self, parent_frame):
        ...
        ttk.Label(self.double_out_frame, text="Bevorzugtes Double-Out:").grid(
            row=0, column=0, sticky=tk.W
        )

        double_values = ["Keine"] + [str(i) for i in range(1, 21)] + ["Bull"]
        self.double_out_combo = ttk.Combobox(
            self.double_out_frame,
            textvariable=self.preferred_double_var,
    ...
    def _on_save(self):
        ...
        if self.profile_to_edit:
            ...
            success = self.profile_manager.update_profile(
                ...
                accuracy_model=accuracy_model_to_save,
            )
        else:
