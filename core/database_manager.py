...
class DatabaseManager:
    ...
    def get_scores(self, game_mode):
        ...
        try:
            with self.Session() as session:
                ...
                results = session.query(Highscore).filter_by(game_mode=game_mode).order_by(sort_func(Highscore.score_metric), desc(Highscore.date)).limit(10).all()
                
                # Konvertiere die ORM-Objekte in Dictionaries für die Kompatibilität mit der UI
                return [
                    {'player_name': r.player_name, 'score_metric': r.score_metric, 'date': r.date}
                    for r in results
                ]
        except SQLAlchemyError as e:
    ...
    def add_score(self, game_mode, player_name, score_metric):
        ...
        if not self.Session: return
        with self.Session() as session:
            new_score = Highscore(game_mode=game_mode, player_name=player_name, score_metric=score_metric, date=date.today())
    ...
    def get_all_player_names_from_records(self):
        ...
        if not self.Session: return []
        with self.Session() as session:
            results = session.query(GameRecord.player_name).distinct().order_by(GameRecord.player_name).all()
    ...
    def get_records_for_player(self, player_name):
        ...
        if not self.Session: return []
        with self.Session() as session:
            results = session.query(GameRecord).filter_by(player_name=player_name).order_by(desc(GameRecord.game_date)).all()
        ...
        with self.Session() as session:
            try:
                new_profile = PlayerProfile(
                    name=name, avatar_path=avatar_path, dart_color=dart_color,
                    is_ai=is_ai, difficulty=difficulty, preferred_double=preferred_double,
                ...
                session.rollback()
                logger.warning(f"Versuch, ein Profil mit dem bereits existierenden Namen '{name}' zu erstellen.")
                return False
            except SQLAlchemyError as e:
                session.rollback()
    ...
    def update_profile(self, profile_id, new_name, new_avatar_path, new_dart_color, is_ai, difficulty, preferred_double, accuracy_model=None):
        ...
        with self.Session() as session:
            try:
                ...
                session.rollback()
                logger.warning(f"Versuch, ein Profil auf einen bereits existierenden Namen '{new_name}' zu aktualisieren.")
                return False
            except SQLAlchemyError as e:
                session.rollback()
                logger.error(f"Fehler beim Aktualisieren des Profils ID {profile_id}: {e}", exc_info=True)
                return False
        ...
        with self.Session() as session:
            try:
                ...
                if profile:
                    ...
                    session.commit()
                    return True
                logger.warning(f"Konnte Genauigkeitsmodell nicht speichern: Profil '{player_name}' nicht gefunden.")
                return False
            except SQLAlchemyError as e:
                session.rollback()
                logger.error(f"Fehler beim Speichern des Genauigkeitsmodells für '{player_name}': {e}", exc_info=True)
                return False
