...
class TournamentStrategyBase:
    ...
    def _prepare_initial_players(self, players: list[str], shuffle: bool = True) -> tuple[list, list]:
...
class SingleEliminationStrategy(TournamentStrategyBase):
    ...
    def _create_initial_bracket(self, players: list[str], shuffle: bool = True) -> dict:
        ...
        return {
            'winners': all_rounds,
            'losers': [],
            'final_bracket': [{'player1': None, 'player2': None, 'winner': None}] # Spiel um Platz 3
        }
...
class DoubleEliminationStrategy(TournamentStrategyBase):
                ...
                if lb_round_idx % 2 == 0:
                    if lb_round_idx == 0: # Erste LB-Runde, gefüttert von WB R1
                        ...
                        if wb_parent1.get('winner') and match_to_fill.get('player1') is None:
                            if loser := self._get_match_loser(wb_parent1): match_to_fill['player1'] = loser
                        if wb_parent2.get('winner') and match_to_fill.get('player2') is None:
                            if loser := self._get_match_loser(wb_parent2): match_to_fill['player2'] = loser
    ...
    def _update_grand_final(self):
        ...
        if wb_final.get('winner') and lb_final and lb_final.get('winner') and not grand_final_matches:
